...
// Timer duration in hours (constant for this code)
const TIMER_HOURS = 24; // 24 hours
...
function showDiscountPopup(expiryTime) {
...
document.addEventListener("click", (e) => {
  if(e.target.id === "closePopup"){
    const popup = document.querySelector("div[id='discountPopup']");
    if(popup) popup.remove();
  }
  if(e.target.id === "reviewBtn") window.open("https://example.com/review", "_blank");
  if(e.target.id === "continueShopBtn") window.location.href = `https://example.com/shop?discount=${DISCOUNT_CODE}`;
});
...
discountBtn.addEventListener("click", () => {
  ...
  if (!expiryTime || now > parseInt(expiryTime)) {
    expiryTime = now + TIMER_HOURS * 60 * 60 * 1000; // constant timer
    localStorage.setItem(STORAGE_KEY, expiryTime);
  } else {
    expiryTime = parseInt(expiryTime);
  }

  showDiscountPopup(expiryTime);
  discountBtn.style.display = "none"; // hide button after first click
});
